...
def load_data(source, target, transforms):
    ...
    datamanager = torchreid.data.ImageDataManager(
        ...
        transforms=transforms  #  transformations from pytorch applied to model training, data augmentation
    )
...
def build_engine(datamanager, model): 
    ...
    # We use adam optimizer, most common used
    # Learning rate to 0.0003
...
if __name__ == "__main__":
    ...
    # data augmentation random_flip and color_jitter fro cross validation
    datamanager = load_data('market1501', ['market1501', 'cuhk03'], ['random_flip', 'color_jitter'])
    
    # Build model to train and evaluate 
    model = build_model(datamanager, 'osnet_x1_0')

    # Build engine with model and data
    engine = build_engine(datamanager, model)

    # Run engine
    run_engine(engine, 'osnet_cross_validation')
